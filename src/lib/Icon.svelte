<script lang="ts">
  /**
   * Inline SVG icon — zero dependency, theme-adaptive (strokes inherit currentColor), offline.
   * The geometry follows the Lucide line-icon vocabulary (24px grid, 2px stroke, round caps) so the
   * set reads as one family. Decorative by default; callers that use an icon as the only label must
   * provide their own title/aria-label on the surrounding control.
   */
  interface Props {
  ...
  // Each entry is the inner markup of a 24x24 viewBox icon. Static constants (no user input),
  // so {@html} below is safe.
  const PATHS = {
    "file-plus":
